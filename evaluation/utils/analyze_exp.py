...
def _trial_metrics(trial: dict) -> dict:
    ...
    rms_u = (float(np.sqrt(np.mean(np.array(actions, float) ** 2)))
...
def controller_summary(name: str, trials: list, n_boot: int = 10_000) -> dict:
    ...
    arr = np.asarray(st_all, float)
    ci_lo, ci_hi = bootstrap_ci(st_all,  n_boot=n_boot)
    sr_lo, sr_hi = bootstrap_ci([1]*n_succ + [0]*(n-n_succ), n_boot=n_boot)
    ...
    return dict(
        ...
        sr_ci_lo=100.0 * sr_lo,
        sr_ci_hi=100.0 * sr_hi,
        n_success=n_succ,
...
def compute_stat_tests(summaries, alpha=0.05):
    ...
    for i, j in combinations(range(n), 2):
        a, b = summaries[i]["settling_all"], summaries[j]["settling_all"]
...
def run_exp1(data: dict, outdir: str, n_boot: int = 10_000):
    ...
    # -- Boxplot -----------------------------------------------------------
    plot_data = [s["settling_all"] for s in summaries]
...
def main():
    ...
    if args.exp == "1":
        if not args.files:
            sys.exit("ERROR: --files required for exp 1")
        ...
        # Experiment 1
        if not args.files:
            sys.exit("ERROR: --files required for exp1 in --exp all")
        data_e1 = load_json(args.files[0])
